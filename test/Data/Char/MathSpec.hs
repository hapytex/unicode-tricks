...
spec = do
    ...
    describe "Serif" do
      describe "NoItalic" do
        describe "NoBold" do
          ...
          it "unsupported" do
            transform <$> unsupported `shouldBe` (Nothing <$ unsupported)
        describe "Bold" do
  ...
  describe "monospace" do
    ...
    it "greek" do
      monospace <$> greek `shouldBe` (Nothing <$ greek)
    it "0-9 (char)" do
      monospace <$> digits `shouldBe` Just <$> "𝟶𝟷𝟸𝟹𝟺𝟻𝟼𝟽𝟾𝟿"
    it "0-9 (int)" do
      intToDigitMonospace <$> [0..9] `shouldBe` Just <$> "𝟶𝟷𝟸𝟹𝟺𝟻𝟼𝟽𝟾𝟿"
    it "unsupported" do
      monospace <$> unsupported `shouldBe` (Nothing <$ unsupported)
  where
    latin, greek, digits, unsupported :: String
    latin = ['A'..'Z'] <> ['a'..'z']
